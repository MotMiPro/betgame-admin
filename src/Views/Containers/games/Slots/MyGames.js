...
import { turnSlotMyGame, withSlotMygameColumns } from "./withColumns";
...
function MyGames() {
  ...
  const handleGetGames = async (query) => {
    try {
      setLoading(true);
      const { data } = await getMyGames(query, authHeader, "user-slot");
      if (data?.success) {
        const { data: list, limit, page, total } = data?.data;
        setTabledata((pagin) => ({
          ...pagin,
          column: withSlotMygameColumns(onFilter),
          data: turnSlotMyGame(list),
          pagination: {
            total: total,
            current: page,
            pageSize: limit,
