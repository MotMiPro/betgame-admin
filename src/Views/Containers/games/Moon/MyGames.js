...
import { turnMoonMygame, withMyMoonColumns } from "./withColumns";
...
function MyGames() {
  ...
  const handleGetGames = async (query) => {
    try {
      setLoading(true);
      const { data } = await getMyGames(query, authHeader, "user-moon");
      if (data?.success) {
        const { data: list, limit, page, total } = data?.data;
        setTabledata((pagin) => ({
          ...pagin,
          column: withMyMoonColumns(onFilter),
          data: turnMoonMygame(list),
          pagination: {
            total: total,
            current: page,
            pageSize: limit,
