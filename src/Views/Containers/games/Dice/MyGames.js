...
import { turnDiceMygame, withMyDiceColumns } from "./withColumns";
...
function MyGames() {
  ...
  const handleGetGames = async (query) => {
    try {
      setLoading(true);
      const { data } = await getMyGames(query, authHeader);
      if (data?.success) {
        const { data: list, limit, page, total } = data?.data;
        setTabledata((pagin) => ({
          ...pagin,
          column: withMyDiceColumns(onFilter),
          data: turnDiceMygame(list),
          pagination: {
            total: total,
            current: page,
            pageSize: limit,
